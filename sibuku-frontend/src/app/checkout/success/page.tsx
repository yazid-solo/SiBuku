...
function ItemLine({ it }: { it: any }) {
  ...
  return (
    <div className="py-3 flex items-center justify-between gap-4">
      ...
      <div className="text-sm font-semibold text-white/80">{formatRupiah(Number.isFinite(subtotal) ? subtotal : 0)}</div>
    </div>
  );
}
...
export default function CheckoutSuccessPage() {
  ...
  const { data: order, isLoading, isError, error, refetch, isFetching } = useQuery({
    ...
    queryFn: () => fetchJson<Order>(`/api/orders/${orderId}`),
    enabled: !!orderId,
    retry: false,
    staleTime: 0,
    refetchInterval: (q) => {
                          ...
                          onClick={async () => {
                            try {
                              ...
                              toast({
                                variant: "success",
                                title: "Tersalin",
                                message: "Kode order berhasil dicopy.",
                              });
                            } catch {
                              toast({
                                variant: "error",
                                title: "Gagal",
                                message: "Tidak bisa akses clipboard.",
                              });
                            }
                          }}
                        >
